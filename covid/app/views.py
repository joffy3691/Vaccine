...
class Final(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            while avb > 0:
                pending = \
                Request.objects.filter(hospital=context['booked_hospital'][0].hospital).filter(fulfilled=0).order_by(
                    'priority')[0]
                pending.fulfilled = 1
                ...
                av.available_beds -= 1
                av.save()
            context['queue'] = Request.objects.filter(fulfilled=0).filter(
                hospital=context['booked_hospital'][0].hospital).count()
            context['index'] = Request.objects.filter(fulfilled=0).filter(
                hospital=context['booked_hospital'][0].hospital).filter(
                priority__lt=context['booked_hospital'][0].priority).count()
        except:
            pass
        context['hospitals'] = Hospital.objects.all()
        return context
...
class Hstatus(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['UserDetails']=User_Attributes.objects.filter(user=context['requests'].user)
        return context
